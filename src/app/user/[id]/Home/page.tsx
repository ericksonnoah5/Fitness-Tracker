...
export default function IdHomePage() {
  ...
  async function myFunction() {
    ...
    changeTotal(String(data?.Total_Cal));
    const num = data?.Total_Cal / 3500;
  ...
  async function addWeight() {
    ...
    await supabase
      .from("Weight")
      .update({ Total_Cal: weightData?.Total_Cal + Number(addsomeWeight) })
      .eq("user_id", userData?.id);
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center pt-20 gap-8">
      <div className="bg-gray-900 rounded-2xl p-8 w-80 flex flex-col gap-4 shadow-xl">
        <h2 className="text-2xl font-bold text-center">{totalWeight} <span className="text-gray-400 text-lg font-normal">calories left</span></h2>
        <h2 className="text-2xl font-bold text-center">{weight} <span className="text-gray-400 text-lg font-normal">lbs to go</span></h2>
      </div>
      <div className="bg-gray-900 rounded-2xl p-8 w-80 flex flex-col gap-2 shadow-xl">
        ...
        {inputs.map((item, i) => (
          <div key={i} className="flex justify-between text-sm text-gray-300 border-b border-gray-800 py-2">
        ...
        <div className="flex justify-between font-semibold pt-2">
          <span>Total</span>
          <span>{inputs.reduce((sum, item) => sum + Number(item.calories), 0)} cal</span>
        </div>
      </div>
      <div className="bg-gray-900 rounded-2xl p-8 w-80 flex flex-col gap-4 shadow-xl">
        <input
